...
class QuantumCircuit:
  def __init__(c,n,m=0):
    ...
    c.data=[('',0)]
  def __add__(c,c2):
    c3=QuantumCircuit(max(c._n,c2._n),max(c._m,c2._m))
  ...
  def measure(c,q,b):
    assert b<c._m, 'Index for output bit out of range.'
    c.data.append(('m',q,b))
  def rz(c,T,q):
    c.data.append(('h',q))
    c.data.append(('rx',T,q))
...
def simulate(c,shots=1024,get='counts'):
  ...
  if g==-1:
    ...
    if g==0:
      assert shots>=4**c._n, 'Use at least shots=4**n to get well-behaved counts in MicroQiskit.'
      return {('{0:0'+str(c._n)+'b}').format(j):p*shots for j,p in enumerate(ps)}
    else:
      m=[]
